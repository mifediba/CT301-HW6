...
#include <stdexcept>
    ...
    void BasicGOL::ToggleCell(int index){
        int size = grid.size();;
    ...
    void BasicGOL::ToggleCell(int row, int col){
        if (row < 0 || row >= height){
    ...
    void BasicGOL::InputContentCheck(int width, int height, std::string grid){
        int size = grid.size();
        if (width <= 0 || height <= 0){
    ...
    std::string BasicGOL::GetGridDirect(std::string board){
        std::string grid;
        for (char c : board){
                if (c == 'X'){
                    grid += "0";
